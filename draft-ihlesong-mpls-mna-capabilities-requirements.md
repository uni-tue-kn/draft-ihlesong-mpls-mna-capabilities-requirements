...
1. In-Stack MNA capabilities:
   ...
   - The NAS Maximum Label Depth (MLD_NAS): the maximum supported NAS length (NASL) for each scope (select, HBH, I2E).
   - The Maximum NAS Count (MNC): the maximum number of NAS of each scope (select, HBH, I2E) that the node can process, where a value of 0 indicates that the scope is not supported.
...
2. Post-Stack MNA capabilities:
   ...
   - The maximum number of PSMHs the node can process (MPC), since a packet may carry one PSMH per scope, where a value of 0 indicates that the node does not support Post-Stack MNA processing as defined in {{!I-D.ietf-mpls-mna-ps-hdr}},
...
All depths, sizes, and offsets in this document are expressed in units of 4 octets, for ISD as well as for PSD.
An MPLS LSE is 4 octets long, and the PSMH-Len, the PS-NAL, and the PSMH offset opcodes of {{!I-D.ietf-mpls-mna-ps-hdr}} are encoded in 4-octet units, so In-Stack and Post-Stack values share one unit and can be compared and added without conversion.
...
| RLD          | Readable Label Depth      | The number of LSEs a node can parse without performance impact.                                                                            | {{!rfc9994}}                  |
| MLD_NAS      | NAS Maximum Label Depth   | The maximum number of Format C and D LSEs in a NAS that a node can process, defined per scope.                                             | This document                 |
| MNC          | Maximum NAS Count         | The maximum number of NAS of a given scope that a node can process, defined per scope. A value of 0 means that the scope is not supported. | This document                 |
...
| MPC          | Maximum PSMH Count        | The maximum number of PSMHs (one per scope) that a node can process. A value of 0 means that Post-Stack MNA is not supported.              | This document                 |
...
An LSR searches the MPLS stack for the NAS that it has to process, so a NAS is only found if it lies within the RLD of that node.
Per Section 7 of {{!rfc9994}}, the entire NAS MUST therefore be placed within the RLD of each participating node of that NAS by the time the packet reaches it.
A NAS beyond the RLD of a participating node means that the node silently does not perform the requested network actions, which the ingress LER cannot detect.

The RLD of a node that does not participate in a NAS, e.g., a node with a low RLD that the operator excludes from a particular network action, does not constrain the placement of that NAS ({{participating}}).
...
### Maximum NAS Sizes (MLD_NAS) {#mld-nas}

#### Motivation
A NAS in the MNA header encoding is at least 2 LSEs and at most 17 LSEs large {{!rfc9994}}, and several NAS of different scopes may be present at an LSR.
With two maximum-sized NAS, an LSR has to reserve 34 LSEs in hardware to be able to process network actions.
This worst case follows from the in-stack encoding itself rather than from a particular hardware architecture: without a means for an LSR to inform the ingress LER that it cannot process maximum-sized NAS, the ingress LER has no basis on which to assume anything smaller, so every LSR has to provision for the worst case irrespective of the NAS sizes actually used in the deployment.

Many use cases of MNA {{?rfc9791}} do not need a NAS of 17 LSEs.
Once the ingress LER can learn the maximum NAS size that a node supports, the resources reserved for NAS can be reduced accordingly, which leaves hardware resources for other purposes, e.g., forwarding labels for SR-MPLS paths, and allows hardware with a low RLD to be made MNA-capable {{IhMe25}}.

#### NAS Maximum Label Depth
The maximum supported number of Format C and Format D LSEs in a NAS that an LSR can process is referred to as NAS Maximum Label Depth (MLD_NAS) in this document.
For each scope in MNA, a separate parameter for the MLD_NAS exists, called MLD_NAS_Select, MLD_NAS_HBH, and MLD_NAS_I2E.
Like the NASL field of {{!rfc9994}}, the MLD_NAS excludes the leading Format A LSE and the Format B LSE that every NAS carries.
A node supports NAS with a NASL of up to its MLD_NAS and such a NAS occupies MLD_NAS + 2 LSEs in the label stack.
The MLD_NAS therefore ranges from 0 to 15, the maximum value of the NASL field; an MLD_NAS of 0 means that the node supports a minimum-sized NAS of 2 LSEs carrying a single opcode without additional data.
This mirrors the MLD_PSMH, which excludes the Post-Stack MPLS Base Header that every PSMH carries ({{mld-psmh}}).

The MLD_NAS is defined per scope because the set of nodes that constrains a NAS differs by scope: a select-scoped NAS is processed by a single node, an HBH-scoped NAS by every participating node, and an I2E-scoped NAS by the egress node.

Based on the MLD_NAS values, the ingress LER has to observe the following when pushing the MPLS stack and NAS on a packet:

- The ingress LER MUST NOT push a select-scoped NAS with a NASL exceeding the MLD_NAS_Select value of the node that processes it.
- The ingress LER MUST NOT push an HBH-scoped NAS with a NASL exceeding the minimum of the MLD_NAS_HBH values of all participating nodes of that NAS.
- The ingress LER MUST NOT push an I2E-scoped NAS with a NASL exceeding the MLD_NAS_I2E value of the egress node.

These constraints apply to the values that the ingress LER has actually learned; where a value is unknown at a participating node, {{unknown}} applies.

The constraints are normative because exceeding a maximum supported value does not degrade gracefully.
An LSR that receives a NAS larger than it can process is unable to complete the network action in the forwarding path, and the packet is either dropped or punted to the slow path, depending on the implementation.
Constructing such stacks therefore results in packet loss or, at sufficient traffic rates, in a denial of service against the control plane of the LSR.
The same applies to the other maxima defined in this document.

#### Example

{{fig-nas_sizes_example}} illustrates the different MLD_NAS values in an MPLS stack.
In this example, the node supports an MLD_NAS_Select of 2, an MLD_NAS_HBH of 5, and an MLD_NAS_I2E of 2, so that the corresponding NAS occupy 4, 7, and 4 LSEs in the label stack.

~~~~
{::include ./drawings/nas_sizes_example.txt}
~~~~
{: #fig-nas_sizes_example title="Example MPLS stack illustrating the per-scope MLD_NAS values."}
...
The MNC constrains the number of NAS of a scope independently of the MLD_NAS, which constrains the length of each individual NAS.
An MNC of 0 means that the node does not support the scope at all, so that the support of a scope does not have to be expressed separately: an MNC_HBH of 1 together with an MLD_NAS_HBH of 0, for example, states that the node processes a single HBH-scoped NAS of the minimum size of 2 LSEs.
...
As for the MLD_NAS, these constraints apply only to the values that the ingress LER has actually learned, and exceeding a value has the consequences described in {{mld-nas}}.
...
Unlike the maxima above, an unsupported opcode has a defined outcome in the data plane: per Section 5.4 of {{!rfc9994}}, a node that does not understand an opcode within a NAS acts according to the Unknown Network Action Handling (U) bit of that NAS, i.e., it skips to the next network action when U is 0 and drops the packet when U is 1.
The U bit makes this case deterministic rather than implementation-specific, but neither outcome performs the requested network action: with U set to 0, the packet is forwarded as if the action had never been requested, which the ingress LER cannot detect, and with U set to 1, the traffic is lost.
Section 2.2 of {{?rfc9789}} accordingly recommends that care be taken not to construct an LSP that traverses nodes that do not support a network action that has to be processed at every hop.

The ingress LER therefore needs to know the set of In-Stack network action opcodes that each participating node supports.
Knowing them, the ingress LER SHOULD refrain from pushing an In-Stack network action for a node that does not support the corresponding opcode, and instead use a supported network action, select a different path, or refrain from using MNA on that path.
If the support for an opcode at a participating node is unknown, the ingress LER treats the opcode as not supported by that node ({{unknown}}).
...
This section defines the parameters that a node has to signal per Section 5.3 of {{!I-D.ietf-mpls-mna-ps-hdr}}.

{{fig-psmh_sizes_example}} gives an overview of the parameters defined in this section.
...
~~~~
{: #fig-psmh_sizes_example title="Overview of the post-stack parameters MLD_PSMH, RLD_PSMH, and MPSO."}
...
The In-Stack capabilities of {{in-stack}} consequently also constrain a deployment that uses Post-Stack network actions exclusively: for each scope with a PSMH, the participating nodes have to report a non-zero MNC for that scope and the accompanying NAS has to be placed within their RLD.
An MLD_NAS of 0 suffices for a NAS that only carries the P bit.

### Maximum Post-Stack MPLS Header Size (MLD_PSMH) {#mld-psmh}
...
Like the PSMH-Len field, it excludes the Post-Stack MPLS Base Header.
The MLD_PSMH applies to each PSMH individually, not to their combined size, which is bounded by the RLD_PSMH instead.
Based on the MLD_PSMH values, the ingress LER has to observe the following:

- The ingress LER MUST NOT add a PSMH with a PSMH-Len exceeding the MLD_PSMH of any participating node of that PSMH.
...
As for the MNC, an MPC of 0 means that the node does not process any PSMH, i.e., that it does not support Post-Stack MNA processing, which is optional for a node.
Whether a node supports Post-Stack MNA therefore does not have to be expressed as a separate capability.

Based on the MPC values, the ingress LER has to observe the following:

- For each node, the number of PSMHs whose scope reaches that node MUST NOT exceed that node's MPC. An HBH-scoped PSMH is processed by every participating node, an I2E-scoped PSMH only by the egress node, and a select-scoped PSMH only by the node it targets.
- In particular, the ingress LER MUST NOT add a PSMH to a packet whose decapsulating node has an MPC of 0, as required by Section 6.1 of {{!I-D.ietf-mpls-mna-ps-hdr}}.

As for the other maxima, exceeding a node's MPC has the consequences described in {{mld-nas}}.

### Readable Label Depth Including Post-Stack MPLS Header (RLD_PSMH)
The "Readable Label Depth including PSMH" of {{!I-D.ietf-mpls-mna-ps-hdr}} extends the RLD of {{!rfc9994}} to include the length of the PSMH and is referred to as RLD_PSMH in this document.
...
A PSMH comprises its Post-Stack MPLS Base Header and the Post-Stack network actions {{!I-D.ietf-mpls-mna-ps-hdr}}, so the RLD_PSMH includes the base header of each PSMH, whereas the MLD_PSMH does not; a PSMH therefore occupies one unit more than its PSMH-Len.
...
Those octets are skipped rather than parsed, and their number is signaled explicitly in the packet by the PSMH Start Offset opcode in the corresponding In-Stack NAS.
How far a node can skip is a capability of its own and is expressed by the MPSO ({{mpso}}) rather than by the RLD_PSMH.
The RLD_PSMH is also not derivable from the RLD: a node that can read a given number of LSEs of the label stack cannot necessarily read the same number of LSEs and a PSMH in addition.
...
Whether a node understands the PSMH Start Offset opcode at all is expressed by its set of supported In-Stack opcodes ({{in-stack-opcodes}}); the MPSO expresses how large a value of that opcode the node can act upon.
Since the offset is signaled explicitly in the packet, the ingress LER checks it against the MPSO independently of the depth check against the RLD_PSMH, and can thus distinguish a node that cannot read deeply enough from a node that cannot skip the intervening post-stack headers.

Based on the MPSO values, the ingress LER has to observe the following:

- The ingress LER MUST NOT encode a PSMH at a start offset larger than the MPSO of any participating node of that PSMH.

### Supported Post-Stack Network Action Opcodes
The Post-Stack network action opcode space (MNA-PS-OP) is 7 bits wide, so up to 128 Post-Stack opcodes can be defined {{!I-D.ietf-mpls-mna-ps-hdr}}, and an LSR does not necessarily support all of them.
As for In-Stack opcodes, an unsupported Post-Stack opcode has a defined outcome: a node MUST respect the Unknown Network Action Handling flag encoded in the corresponding NAS when it processes the PSMH {{!I-D.ietf-mpls-mna-ps-hdr}}, so the considerations of {{in-stack-opcodes}} apply.
Since the U bit of a NAS applies jointly to its In-Stack and Post-Stack network actions, the ingress LER cannot choose a different failure mode for the two.
...
If the support for a Post-Stack opcode at a participating node is unknown, the ingress LER treats the opcode as not supported by that node ({{unknown}}).
...
Which nodes participate in a NAS follows from its scope and from the deployment:
...
A node that does not participate contributes no capability values to the constraints of that NAS, but it may still constrain the label stack: per Section 7 of {{!rfc9994}}, a NAS MUST NOT appear at the top of the label stack at an MNA-incapable node, independently of any capability value.
The ingress LER consequently needs to know which nodes on a path are MNA-capable and which of them participate in a given network action, in addition to the capability values of the participating nodes.
...
- A NAS of a given scope can be used only if the participating nodes of that NAS report a non-zero MNC for the scope. These are the targeted node alone for a select-scoped NAS, the egress node for an I2E-scoped NAS, and every node expected to process it for an HBH-scoped NAS. A node that does not support the select scope therefore only rules out select-scoped NAS targeted at itself, whereas a node that does not support the HBH scope rules out HBH-scoped NAS on the whole path.
...
For the combinations above to be correct, the ingress LER has to know the capabilities of exactly the participating nodes of each NAS, since missing information about a single participating node invalidates the path-wide constraint.
...
In particular, an opcode or a scope whose support at a participating node is unknown is treated as not supported by that node.
...
If it can steer traffic, e.g., using SR-TE candidate paths or RSVP-TE explicit routes, it can also select an alternate candidate path or, where several candidate egress nodes exist, an egress node that provides the required capabilities, e.g., the required MLD_NAS_I2E or Post-Stack decapsulation support.
...
If the ingress LER cannot establish that it has the capabilities of every branch, it MUST treat the capabilities of the nodes on the branches that it has not covered as unknown and fall back conservatively ({{unknown}}).
A minimum taken over the known branches is then not a valid path-wide constraint, and an opcode or a scope supported on the known branches is not thereby supported on the LSP.
...
This does not change the requirement, since the ingress LER needs the capabilities that hold for every branch a flow may take rather than those of the branch taken by an individual flow.
A mechanism therefore has to enable either the coverage of every branch of an ECMP set or the ingress LER's ability to detect that coverage is incomplete, not the identification of the branch that a given flow takes.
...
A node that does not support MNA MUST be treated as not supporting any scope, opcode, or Post-Stack processing, i.e., as reporting an MNC of 0 for every scope, so it never participates in a NAS, but it still restricts where a NAS may be placed in the label stack ({{participating}}).
...
- REQ-1: The mechanism MUST allow the ingress LER to learn the RLD of each participating node on a path.
- REQ-2: The mechanism MUST allow the ingress LER to learn, per scope (select, HBH, I2E), both the maximum NAS size (MLD_NAS) and the maximum number of NAS (MNC) that each participating node can process. Since an MNC of 0 indicates that the node does not support the scope, no separate indication of the supported scopes is needed.
...
- REQ-4: The mechanism MUST allow the ingress LER to learn, for each participating node, its MLD_PSMH, the maximum number of PSMHs it can process (MPC), its RLD_PSMH, its MPSO, and its supported Post-Stack network action opcodes. Since an MPC of 0 indicates that the node does not support Post-Stack MNA, no separate indication of Post-Stack MNA support is needed.
- REQ-5: The mechanism MUST allow the ingress LER to associate the learned capabilities with the forwarding path and to determine whether it has the capabilities of all participating nodes a flow may traverse, including across ECMP sets. Full coverage of every ECMP branch is desirable but not required; where it is not achieved, the ingress LER falls back conservatively ({{ecmp}}).
...
| Node | RLD | MLD_NAS_Select | MLD_NAS_HBH | MLD_NAS_I2E | MPC | MLD_PSMH | RLD_PSMH | MPSO |
| ---- | --- | -------------- | ----------- | ----------- | --- | -------- | -------- | ---- |
| R1   | 20  | 9              | 9           | 9           | 1   | 16       | 36       | 1    |
| R2   | 51  | 9              | 3           | 9           | 1   | 8        | 59       | 0    |
| R3   | 35  | 9              | 9           | 9           | 2   | 16       | 51       | 1    |
...
Only the MLD_NAS_I2E of R3 is relevant on this path, since R1 and R2 are not the egress node and therefore never process an I2E-scoped NAS.
...
The MPC values reflect the PSMHs that each node sees: the transit nodes the HBH-scoped PSMH and the egress node both the HBH-scoped and the I2E-scoped PSMH.

Applying the derivation of path-wide constraints from {{aggregation}}, R0 determines:

- Path-wide RLD: min(20, 51, 35) = 20
- Path-wide MLD_NAS_HBH: min(9, 3, 9) = 3
- MLD_NAS_Select for R2: 9
- MLD_NAS_I2E: 9 (from R3)
- Post-Stack MNA is supported on all nodes, since none of them reports an MPC of 0.
- Path-wide MLD_PSMH for an HBH-scoped PSMH: min(16, 8, 16) = 8
- MLD_PSMH for an I2E-scoped PSMH: 16 (from R3)
- Path-wide RLD_PSMH: min(36, 59, 51) = 36
...
R0 can now construct a label stack that observes these constraints.
The two PSMHs additionally require an In-Stack NAS with the P bit set for each of their scopes ({{psd-isd}}).
For the HBH-scoped PSMH, this is the HBH-scoped NAS that R0 pushes anyway, whose Format C and D LSEs remain bounded by the path-wide MLD_NAS_HBH of 3.
For the I2E-scoped PSMH, R0 has to add an I2E-scoped NAS even though it uses no In-Stack network action of that scope; a minimum-sized NAS of 2 LSEs carrying the No-Operation opcode with the P bit set suffices, i.e., a NASL of 0, which any MLD_NAS_I2E admits.
...
A mechanism also has to protect against forged or manipulated capability information, because incorrect capabilities can cause the ingress LER to construct label stacks that a node cannot process, leading to silently dropped network actions, packet loss, or punting of packets to the slow path, and thus to a denial of service against the control plane of an LSR ({{mld-nas}}).
